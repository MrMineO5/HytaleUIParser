...
import app.ultradev.hytaleuiparser.types.Anchor
import app.ultradev.hytaleuiparser.types.Padding
import app.ultradev.hytaleuiparser.types.PatchStyle
...
class UITransformer(val root: RootNode) {
    ...
    private fun createUIElement(node: AstNode, type: ElementType, properties: Map<String, VariableValue>, parentBox: RenderBox): RenderBox {
        val anchor = properties["Anchor"]?.let(Anchor::fromVariable) ?: Anchor.NONE
        val padding = properties["Padding"]?.let(Padding::fromVariable) ?: Padding.NONE

        val selfBox = parentBox.withAnchor(anchor)
        val childBox = selfBox.withPadding(padding)

        val background = properties["Background"]?.let(PatchStyle::fromVariable) ?: PatchStyle.NONE
